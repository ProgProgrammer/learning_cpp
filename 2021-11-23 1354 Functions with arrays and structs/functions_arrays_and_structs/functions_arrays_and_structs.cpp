...
{                                // лушче, чтобы по названию переменной сразу было понятно что это такое, я бы назвал studentsArray или просто students
    ...
    for (i = 0; i < n; i++)
    {
        ...
        {
            strcpy_s(pa[i].fullname, SLEN, str.c_str());  // на заметку - тут ограничение в 30 символов. Наверное это нормально для твоей задачи, 
        }                                               // в другом коде может потребоваться неограниченное количество символов. (ИСПРАВИЛ 30 НА SLEN)
        else
        {
            break;
        }
        ...
        {                                   // Это так и должно быть?
            strcpy_s(pa[i].hobby, SLEN, str.c_str());
        }
